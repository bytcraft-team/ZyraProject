...
class OnboardingViewModel extends ChangeNotifier {
  ...
  Future<bool> completeOnboardingAndGenerateFirstCycle() async {
    ...
    try {
      ...
      if (passedDaysSinceStart >= _cycleDuration) {
        final int cyclesPassed = (passedDaysSinceStart / _cycleDuration).floor();
        adjustedStartDate = _lastPeriodDate!.add(Duration(days: cyclesPassed * _cycleDuration));
      }
      ...
      final DateTime predictedOvulation = adjustedStartDate.add(Duration(days: _cycleDuration - 14));
      ...
      await db.transaction((txn) async {
        ...
        await txn.insert(
          'cycles',
          {
            ...
            'predicted_ovulation': firstCycle.predictedOvulation.toIso8601String(),
            'predicted_fertility_start': firstCycle.predictedFertilityStart.toIso8601String(),
            'predicted_fertility_end': firstCycle.predictedFertilityEnd.toIso8601String(),
        ...
        for (int i = 1; i <= daysInMonth; i++) {
          ...
          final int dayInCycle = CycleDateUtils.daysBetween(firstCycle.startDate, targetDate) + 1;

          if (dayInCycle > 0 && dayInCycle <= _cycleDuration) {
            final isPredicted = targetDate.isAfter(CycleDateUtils.dateOnly(now));
            
            await txn.insert(
              'daily_logs',
              {
                'date': CycleDateUtils.storageKey(targetDate),
                'basal_temperature': isPredicted ? null : 36.5 + (dayInCycle * 0.01),
                'flow_intensity': dayInCycle <= _periodDuration ? 'medium' : null,
      ...
      if (user != null) {
        ...
        debugPrint('🔒 Firebase Auth absent lors de l’onboarding : écriture Firestore différée.');
      }
  ...
  Future<void> _syncToFirebaseBackground(String uid, UserModel user, CycleModel cycle) async {
    try {
      ...
      batch.set(cycleRef, cycle.toFirestoreMap());
